...
const Generate  = () => {
  ...
  const [colorSchemaId, setColorSchemeId] = useState<string>(colorSchemes[0].id)
          ...
          <div className="grid lg:grid-cols-[400px_1fr] gap-8">
            ...
            <div className={`space-y-6 ${id && 'pointer-events-none'}`}>
              <div className="p-6 rounded-2xl bg-white/8 border border-white/12 shadow-xl space-y-6">
                <div>
                  <h1 className="text-xl font-bold text-zinc-100 mb-1">Create your Thumbnail</h1>
                ...
                {/* colorscheaselector */}
                <ColorSchemeSelector value={colorSchemaId} onChange={setColorSchemeId} />

                <div className="space-y-2">
                  <label className="text-sm block font-medium">Additional Prompts {" "}
                    <span className="text-zinc-400 text-xs">(optional)</span>
                    <textarea value={additionalDetails} onChange={(e) => setAdditionalDetails(e.target.value)} rows={3} placeholder="Add any specific elements, mood or style preferences" className="mt-2 w-full px-4 py-3 rounded-lg border border-white/10 bg-white/6 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none" />
                  </label>
                </div>
